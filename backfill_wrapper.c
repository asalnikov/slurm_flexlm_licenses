...
static pthread_t dynamic_licenses_thread = 0;
...
int init( void )
{
	...
	if (pthread_create( &dynamic_licenses_thread, &attr, dynamic_licenses_agent, NULL))
                error("Unable to start dynamic licenses thread: %m");
	pthread_mutex_unlock( &thread_flag_mutex );
...
void fini( void )
{
	...
	if ( dynamic_licenses_thread ) {
                verbose( "Dynamic licensing plugin shutting down" );
                stop_dynamic_licenses_agent();
                pthread_join(dynamic_licenses_thread, NULL);
                dynamic_licenses_thread= 0;
        }
	pthread_mutex_unlock( &thread_flag_mutex );
}
